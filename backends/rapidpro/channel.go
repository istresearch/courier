...
func getChannelByAddress(ctx context.Context, db *sqlx.DB, channelType courier.ChannelType, channelAddress string) (*DBChannel, error) {
	channel := &DBChannel{Address_: sql.NullString{String: channelAddress, Valid: true}}

	// select just the fields we need
	err := db.GetContext(ctx, channel, lookupChannelFromAddressSQL, channelAddress)
